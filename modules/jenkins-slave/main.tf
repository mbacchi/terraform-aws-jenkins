...
data "aws_ami" "jenkins_linux_slave" {
  ...
  filter {
    name   = "name"
    values = ["brave-jenkins-ubuntu-worker-*"]
  }
}
...
resource "aws_instance" "ec2_jenkins_slave" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "ssh -oStrictHostKeyChecking=no -i /tmp/key.pem ${var.user}@${var.jenkins_master_ip} 'sudo cat /var/lib/jenkins/secrets/initialAdminPassword' > /tmp/secret",
